...
```
  ...
  Fixed from last round:{fixed_count}
  Total open:           {open_count} ({critical_count} critical, {high_count} high, {medium_count} medium)
  Skipped (design):     {skipped_count total}
```

### Phase 3: DECIDE

- **No OPEN issues (CRITICAL, HIGH, or MEDIUM)** → proceed to Finalize
...
```
  Decision:             {CLEAN → finalizing / FIXING N issues / STUCK → finalizing / MAX_ROUNDS → finalizing}
```
...
**Early exit**: If zero fixes were actually applied this round (all issues skipped by design constraint), the code is unchanged — running another review would produce identical results. Proceed directly to Finalize instead of returning to Phase 1.

Return to Phase 1 for next round.
...
```
  Simplify:             {X improvements applied}
  Build:                {PASS / FAIL → fixing... (attempt {n}/3)}
```
...
```
  Tests consolidated:   {added X, removed X, updated X}
  Test run:             {PASS / FAIL → fixing... (attempt {n}/3)}
```
...
```
  Test review:          {PASS / X issues → fixing... (attempt {n}/2)}
  Final test run:       PASS
```
...
====================
Rounds: {N} | Result: {CLEAN / STUCK / MAX_ROUNDS}
Fixed: {X} | Remaining: {X} | Design overrides: {X} | Skipped by design: {X}
...
DESIGN OVERRIDES (if any):
  [R{n}] {file}:{line} — {what was changed and why it overrode design.md}
